# 🧠 德州扑克AI | AI源码| 德州最强AI| 1对1无限注 |德州AI源码| 可战胜职业牌手 | 完整源码+训练模型

> **CFR算法 + 深度神经网络 + 蓝图搜索 | C++实现 | 全套AI源代码**

[![Contact](https://img.shields.io/badge/联系-TG%3A%40xuzongbin001-blue)](https://t.me/xuzongbin001)
[![Platform](https://img.shields.io/badge/平台-Linux%20%7C%20Windows-green)]()
[![Language](https://img.shields.io/badge/语言-C%2B%2B-red)]()

---

## 🏆 核心特色

| 特色模块 | 说明 |
| :--- | :--- |
| ♠️ **1对1无限注** | Heads-Up No-Limit Texas Hold'em |
| 🧠 **CFR算法** | 反事实遗憾最小化，收敛至纳什均衡 |
| 🔍 **蓝图搜索** | FOLD/CALL/RAISE/DEFAULT四蓝图决策 |
| 📊 **深度神经网络** | 评估博弈状态与行动价值 |
| 💪 **战胜职业牌手** | AI已达到高级职业水平 |
| 📦 **完整交付** | 源码 + 训练模型 + 部署脚本 |

## 🎯 AI能力

| 能力 | 说明 |
| :--- | :--- |
| **决策速度** | 毫秒级响应 |
| **策略深度** | GTO最优策略 |
| **自适应** | 可根据对手调整 |
| **训练数据** | 大规模自我对弈 |

## 🚀 技术架构

| 层级 | 技术 |
| :--- | :--- |
| **核心算法** | CFR + 深度有限搜索 |
| **决策蓝图** | 4种行动蓝图 |
| **评估网络** | 深度神经网络 |


## 📸 系统截图

| AI对战界面 | 决策过程 | 训练曲线 |
| :---: | :---: | :---: |
<img width="675" alt="微信图片_20241203165109" src="https://github.com/user-attachments/assets/b2ea9024-e134-4d6d-a0a6-7cb7ca00afe9" />
<img width="709" alt="微信图片_20241203165100" src="https://github.com/user-attachments/assets/fc89e206-551b-4575-89f6-9b91f989cc93" />
<img width="453" alt="微信图片_20241203165044" src="https://github.com/user-attachments/assets/89a8ff65-fb7b-47ee-97d4-e9810b311538" />
<img width="379" alt="微信图片_20241030112757" src="https://github.com/user-attachments/assets/997635b4-b6cb-43c1-9e53-ec9e026a4585" />

![640](https://github.com/user-attachments/assets/d5ce886a-d8bd-49ad-be83-48f09e79c8cd)


![640 (1)](https://github.com/user-attachments/assets/dbd09a91-fa9c-47f7-b8ef-dfeae0a136ce)
> 📷 更多截图和演示视频，联系我获取

🎥 **演示视频**：[联系我获取在线演示](https://t.me/xuzongbin001)

## 💰 获取源码

✅ 完整C++ AI源码  
✅ 训练好的AI模型  
✅ 部署脚本  
✅ 技术文档  

📱 **Telegram：@xuzongbin001**  
📧 **Email：masterai918@gmail.com**

👉 **联系我获取演示 + 详细报价**

---

⭐ Star 这个仓库，支持世界级德州AI持续进化！




